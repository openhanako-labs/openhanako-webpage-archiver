...
| 工具 | 功能 |
|------|------|
| `webpage_archiver_save` | 保存网页为离线单文件 HTML（crawl4ai 获取 + SingleFile 打包） |
...
- **crawl4ai**: `pip install crawl4ai`（统一联网抓取 + 内容提取）
...
- Node.js ESM
- Python（Crawl4AI，通过 mkdtemp 临时目录隔离执行）
...
- v2.2: 统一联网抓取路径（crawl4ai 为主），临时文件隔离清理（mkdtemp），隐私规则库外部化，工具间 archiveId 契约
