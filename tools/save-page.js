...
const description = "保存网页为离线单文件 HTML。使用 SingleFile CLI 将完整网页（含图片、CSS、字体）打包为单个 HTML 文件。触发词：保存网页、存档页面、save page、SingleFile。";
...
async function execute(input) {
  try {
    const { url, outputDir, filename, blockImages, waitUntil } = input;

    // 构建输出路径
    const os = await import("node:os");
    const fs = await import("node:fs");
    const crypto = await import("node:crypto");
    const dir = outputDir || path.join(os.tmpdir(), "hanako-singlefile");
